...
1. IShRemote PowerShell cmdlets can be recognized by their naming convention as the noun starts with `ISh`, so verb-`ISh`noun like `New-IShSession`. Most of the cmdlets of IShRemote are made available as MCP Tools with the same or reduced parameter set - for example parameter `-IshSession` with value `$ishSession` is superfluous and should be avoided. Do not pass parameter `-IshSession` to any MCP Tool.
2. All ISHRemote MCP Tools are allowed to be loaded, use `Get-Help` to help resolve the request. Do not make the user individually activate groups of MCP Tools.
3. Do not make up parameters for the cmdlets! Do not use any JSON parameters! Do not use hash table structures (`@{ Name = "FUSERNAME"; Value = ...}`) for ISHRemote cmdlets! All parameters should exist and can be verified over the MCP Tool description or by triggering ISHRemote MCP Tool `Get-Help` cmdlet on it.
...
8. All IShRemote cmdlets come with preformatted tables, there is no need to pipeline (`|`) these results in a `Format-Table` cmdlet.
