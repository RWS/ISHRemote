...
function Invoke-IshRemoteMcpHandleRequest {
    ...
    if ($Request.method -eq "initialize") {
        ...
        $response = '{"jsonrpc":"2.0","id":' + ($Request.id | ConvertTo-Json -Depth 10 -Compress) + ',"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{"listChanged":false}},"instructions":' + $instructionsJson + ',"serverInfo":{"name":"ISHRemote MCP Server","version":"0.3.0"}}}'
        return $response
    }
