        ...
        protected override void EndProcessing()
        {
            try
            {
                ...
                if (ParameterSetName == "IshObjectsGroup")
                {
                   var ishObjectsDividedInBatches = DevideListInBatchesByLogicalId(_retrievedIshObjects, IshSession.MetadataBatchSize);
                    ...
                    foreach (var ishObjectsGroup in ishObjectsDividedInBatches)
                    {
                        ...
                        var response = IshSession.DocumentObj25.RaiseEventByIshLngRefs(new DocumentObj25ServiceReference.RaiseEventByIshLngRefsRequest()
                        {
                            psAuthContext = IshSession.AuthenticationContext,
                            palLngRefs = lngCardIds.ToArray(),
                            psEventType = EventType
                        });
                        IshSession.AuthenticationContext = response.psAuthContext;
                        progressIds.Add(response.plOutProgressId);
                ...
                if (ParameterSetName == "ParameterGroup")
                {
                    ...
                    {
                        psAuthContext = IshSession.AuthenticationContext,
                        psDescription = EventDescription,
                        psEventType = EventType,
                        piMaximumProgress = _startEventMaxProgress
                    });
                    IshSession.AuthenticationContext = responseStartEvent.psAuthContext;
                }
                ...
                { 
                    ...
                    var responseStartAfterEvent = IshSession.BackgroundTask25.CreateBackgroundTaskWithStartAfter(new CreateBackgroundTaskWithStartAfterRequest()
                    {
                        ...
                        psEventType = EventType,
                        psHashId = "",
                        pbInputData = Encoding.Unicode.GetBytes(RawInputData),
                        pdStartAfter = StartAfter.Value
                    });
                    IshSession.AuthenticationContext = responseStartAfterEvent.psAuthContext;
                    progressIds.Add(responseStartAfterEvent.plOutProgressId);
                }
                ...
                var responseFind = IshSession.BackgroundTask25.Find(new FindRequest()
                {
                    psAuthContext = IshSession.AuthenticationContext,
                    pdModifiedSince = _modifiedSince,
                    peUserFilter = _userFilter,
                    psXMLMetadataFilter = metadataFilter.ToXml(),
                    psXMLRequestedMetadata = requestedMetadata.ToXml()
                });
                IshSession.AuthenticationContext = responseFind.psAuthContext;
                var xmlIshBackgroundTasks = responseFind.psOutXMLTaskList;
