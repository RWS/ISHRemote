            ...
            try
            {
                ...
                if (IshFolder != null)
                {
                    ...
                    foreach (IshFolder ishFolder in ishFolders)
                    {
                        ...
                        if (readAccess == null)
                        {
                            ...
                            {
                                var responseRename = IshSession.Folder25.Rename(new Folder25ServiceReference.RenameRequest()
                                {
                                    psAuthContext = IshSession.AuthenticationContext,
                                    plFolderRef = folderId,
                                    psNewFolderName = newFolderName
                                });
                                IshSession.AuthenticationContext = responseRename.psAuthContext;
                            }
                        }
                        else
                        {
                            ...
                            if (ShouldProcess(Convert.ToString(folderId)))
                            {
                                var responseUpdate = IshSession.Folder25.Update(new Folder25ServiceReference.UpdateRequest()
                                {
                                    psAuthContext = IshSession.AuthenticationContext,
                                    plFolderRef = folderId,
                                    psNewFolderName = newFolderName,
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (ShouldProcess(Convert.ToString(folderId)))
                        {
                            var responeUpdate = IshSession.Folder25.Update(new Folder25ServiceReference.UpdateRequest()
                            { 
                                psAuthContext = IshSession.AuthenticationContext,
                                plFolderRef = folderId,
                                psNewFolderName = newFolderName,
                            ...
                            IshSession.AuthenticationContext = responeUpdate.psAuthContext;
                        }
                    }
                    returnFolderIds.Add(folderId);
                }
                ...
                {
                    psAuthContext = IshSession.AuthenticationContext,
                    palFolderRefs = returnFolderIds.ToArray(),
