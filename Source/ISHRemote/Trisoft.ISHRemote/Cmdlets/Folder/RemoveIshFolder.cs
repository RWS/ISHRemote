            ...
            try
            {
                ...
                if (IshFolder != null)
                {
                    ...
                    foreach (IshFolder ishFolder in ishFolders)
                    {
                        ...
                        if (!Recurse)
                        {
                            ...
                            {
                                var responseDelete = IshSession.Folder25.Delete(new Folder25ServiceReference.DeleteRequest()
                                { 
                ...
                else if (FolderId != 0)
                {
                    ...
                    if (!Recurse)
                    {
                        if (ShouldProcess(Convert.ToString(folderId)))
                        {
                            var response = IshSession.Folder25.Delete(new Folder25ServiceReference.DeleteRequest()
                            { 
                                psAuthContext = IshSession.AuthenticationContext,
                                plFolderRef = folderId
                            });
                    ...
                    else
                    {
                        ...
                        string xmlIshFolder = "";
                        var response = IshSession.Folder25.GetMetaDataByIshFolderRef(new Folder25ServiceReference.GetMetaDataByIshFolderRefRequest()
                        {
                            psAuthContext = IshSession.AuthenticationContext,
                            plFolderRef = folderId,
