...
Describe "Get-IshFolderLocation" -Tags "Read" {
	...
	Context "Get-IshFolderLocation returns string object" {
		It "GetType().Name" {
			...
			$ishObjects.GetType().Name | Should-BeString -CaseSensitive "String"
		}
	}
	Context "Get-IshFolderLocation BaseFolderGroup" {
		...
		It "Parameter BaseFolder Data" {
			Get-IshFolderLocation -IShSession $ishSession -BaseFolder Data | Should-NotBeNull
		}
		It "Parameter BaseFolder System" {
			Get-IshFolderLocation -IShSession $ishSession -BaseFolder System | Should-NotBeNull
		}
		It "Parameter BaseFolder Favorites" {
			Get-IshFolderLocation -IShSession $ishSession -BaseFolder Favorites | Should-NotBeNull
		}
		It "Parameter BaseFolder EditorTemplate" {
			Get-IshFolderLocation -IShSession $ishSession -BaseFolder EditorTemplate | Should-NotBeNull
		}
	}
	Context "Get-IshFolderLocation FolderPathGroup" {
		It "Parameter FolderPath invalid" {
			$exception = { Get-IshFolderLocation -IShSession $ishSession -FolderPath "INVALIDFOLDERPATH" } | Should-Throw
	...
	Context "Get-IshFolderLocation FolderIdsGroup" {
		...
		It "Parameter FolderId from EditorTemplate" {
			Get-IshFolderLocation -IshSession $ishSession -FolderId $ishFolderEditorTemplateFolderRef | Should-NotBeNull
		}
		It "Parameter FolderId 0 no longer returns BaseFolder" {
			$exception = { Get-IshFolderLocation -FolderId 0 } | Should-Throw
