...
Describe "Get-IshMetadataField" -Tags "Read" {
	...
	Context "Get-IshMetadataField -IshSession ishSession returns String object" {
		It "GetType().Name" {
			...
			$ishFolderData.GetType().Name | Should-BeString -CaseSensitive "String"
		}
	}
	Context "Get-IshMetadataField -IshSession ishSession IshFieldsGroup" {
		...
		It "Parameter IshFolder FNAME" {
			Get-IshMetadataField -IshSession $ishSession -Name "FNAME" -IshField $ishFolderSystemOriginal.IshField | Should-NotBeNull
		}
		...
		It "Parameter IshFolder READ-ACCESS with implicit IshSession" {
			Get-IshMetadataField -Name "READ-ACCESS" -Level None -IshField $ishFolderSystemOriginal.IshField -ValueType Element | Should-BeEmptyString
		}
	}
