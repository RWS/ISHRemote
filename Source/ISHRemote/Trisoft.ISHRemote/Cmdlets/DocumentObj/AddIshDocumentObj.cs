            ...
            try
            {
                ...
                if (IshObject != null)
                {
                    ...
                    foreach (IshObject ishObject in IshObject)
                    {
                        ...
                        DocumentObj25ServiceReference.CreateResponse responseCreate = null;
                        if (ShouldProcess(logicalId + "=" + ishObjectVersion + "=" + ishObjectLanguage + "=" + ishObjectResolution))
                        {
                            responseCreate = IshSession.DocumentObj25.Create(new DocumentObj25ServiceReference.CreateRequest()
                            {
                                psAuthContext = IshSession.AuthenticationContext,
                                plFolderRef= _folderId,
                                psIshType = ishObject.IshType.ToString(),
                                psLogicalId = logicalId,
                                psVersion = ishObjectVersion,
                                psLanguage = ishObjectLanguage,
                                psResolution = ishObjectResolution,
                        ...
                        var responseGet = IshSession.DocumentObj25.GetMetaData(new DocumentObj25ServiceReference.GetMetaDataRequest()
                        {
                            psAuthContext = IshSession.AuthenticationContext,
                            psLogicalId = responseCreate.psLogicalId,
                            psVersion = responseCreate.psVersion,
                            psLanguage = ishObjectLanguage,
                            psResolution = ishObjectResolution,
                ...
                else
                {
                    ...
                    {
                        var responseCreate = IshSession.DocumentObj25.Create(new DocumentObj25ServiceReference.CreateRequest()
                        {
                            psAuthContext = IshSession.AuthenticationContext,
                            plFolderRef = _folderId,
                            psIshType = IshType.ToString(),
                            psLogicalId = LogicalId,
                            psVersion = Version,
                            psLanguage = Lng,
                            ...
                            psXMLMetadata = metadata.ToXml(),
                            psEdt = _ishData.Edt,
                            pbData = _ishData.ByteArray
                        });
                        IshSession.AuthenticationContext = responseCreate.psAuthContext;
                        IshFields requestedMetadata = IshSession.IshTypeFieldSetup.ToIshRequestedMetadataFields(IshSession.DefaultRequestedMetadata, ISHType, metadata, Enumerations.ActionMode.Read);
                        var response = IshSession.DocumentObj25.GetMetaData(new DocumentObj25ServiceReference.GetMetaDataRequest()
                        {
                            psAuthContext = IshSession.AuthenticationContext,
                            psLogicalId = responseCreate.psLogicalId,
                            psVersion = responseCreate.psVersion,
