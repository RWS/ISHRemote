                ...
                else
                {
                    ...
                    if (IshObject != null)
                    {
                        ...
                        if (!_includeData)
                        {
                            ...
                            foreach (List<long> lngCardIdBatch in devidedlngCardIdsList)
                            {
                                ...
                                var response = IshSession.DocumentObj25.RetrieveMetadataByIshLngRefs(new DocumentObj25ServiceReference.RetrieveMetadataByIshLngRefsRequest()
                                {
                                    ...
                                    palLngRefs = lngCardIdBatch.ToArray(),
                        ...
                        else
                        {
                            ...
                            {
                                // Process language card ids in batches
                                var response = IshSession.DocumentObj25.RetrieveObjectsByIshLngRefs(new DocumentObj25ServiceReference.RetrieveObjectsByIshLngRefsRequest()
                                {
                                    psAuthContext = IshSession.AuthenticationContext,
                                    palLngRefs = lngCardIdBatch.ToArray(),
                                    psXmlProductDefinition = productDefinitionFeatures.ToXml(),
                                    psXMLRequestedMetadata = requestedMetadata.ToXml()
                                });
                                IshSession.AuthenticationContext = response.psAuthContext;
                                string xmlIshObjects = response.psOutXMLObjList;
                    ...
                    else
                    {
                        ...
                        var statusFilter = EnumConverter.ToStatusFilter<DocumentObj25ServiceReference.eISHStatusgroup>(StatusFilter);
                        if (!_includeData)
                        {
                            //RetrieveMetadata
                        ...
                        else
                        {
                            ...
                            {
                                // Process language card ids in batches
                                var response = IshSession.DocumentObj25.RetrieveObjects(new DocumentObj25ServiceReference.RetrieveObjectsRequest()
                                {
                                    psAuthContext = IshSession.AuthenticationContext,
                                    pasLogicalIds = logicalIdBatch.ToArray(),
                                    peStatusFilter = statusFilter,
