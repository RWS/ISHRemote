            ...
            try
            {
                ...
                if (ParameterSetName == "IshObjectsGroup")
                {
                    ...
                    foreach (IshObject ishObject in ishObjects)
                    {
                        ...
                        if (ishObject.IshData != null)
                        {
                            ...
                            {
                                var response = IshSession.DocumentObj25.UpdateByIshLngRef(new UpdateByIshLngRefRequest()
                                {
                                    ...
                                    pbData = ishObject.IshData.ByteArray
                                });
                        ...
                        else
                        {
                            ...
                            if (ShouldProcess(Convert.ToString(lngRef)))
                            {
                                var response = IshSession.DocumentObj25.SetMetadataByIshLngRef(new SetMetadataByIshLngRefRequest()
                                {
                                    psAuthContext = IshSession.AuthenticationContext,
                                    plLngRef = lngRef,
                                    psXMLMetadata = metadata.ToXml(),
                    ...
                    var responseRetrieve = IshSession.DocumentObj25.RetrieveMetadataByIshLngRefs(new RetrieveMetadataByIshLngRefsRequest()
                    {
                        psAuthContext = IshSession.AuthenticationContext,
                        palLngRefs = lngCardIds.ToArray(),
                ...
                else
                {
                    ...
                    if (ParameterSetName == "ParameterGroupMetadata")
                    {
                        ...
                        {
                            var responseSet = IshSession.DocumentObj25.SetMetadata(new SetMetadataRequest()
                            {
                                psAuthContext = IshSession.AuthenticationContext,
                            ...
                            IshSession.AuthenticationContext = responseSet.psAuthContext;
                            version = responseSet.psVersion;
                        }
                    }
                    else
                    {
                        ...
                        if (ShouldProcess(LogicalId + "=" + Version + "=" + Lng + "=" + resolution))
                        {
                            var responseUpdate = IshSession.DocumentObj25.Update(new UpdateRequest()
                            {
                                psAuthContext = IshSession.AuthenticationContext,
                                psLogicalId = LogicalId,
                                psVersion = Version,
                                ...
                                psXMLRequiredCurrentMetadata = requiredCurrentMetadata.ToXml(),
                                psEdt = ishData.Edt,
                                pbData = ishData.ByteArray
                            });
                            IshSession.AuthenticationContext = responseUpdate.psAuthContext;
                            version = responseUpdate.psVersion;
                        }
                    }
                    ...
                    var responseGet = IshSession.DocumentObj25.GetMetaData(new GetMetaDataRequest()
                    {
                        psAuthContext = IshSession.AuthenticationContext,
                        psLogicalId = LogicalId,
                        psVersion = version,
                    ...
                    IshSession.AuthenticationContext = responseGet.psAuthContext;
                    string xmlIshObjects = responseGet.psOutXMLObjList;
                    IshObjects retrievedObjects = new IshObjects(ISHType, xmlIshObjects);
                    returnedObjects.AddRange(retrievedObjects.Objects);
                }
