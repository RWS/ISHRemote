...
* Introduced cross-platform .NET 10.0 for usage in cross-platform PowerShell 7.6. This variation received all possible third-party library updates as well; ranging from Duende.* over System.ServiceModel.* to supporting libraries like System.Text.Json and more. #235
...
The `ISHRemoteMcpServer` runs on your local machine that pushes the heavy lifting via PowerShell over ISHRemote and in turn HTTPS to a Tridion Docs service. When you ask for `Create a new ishsession to https://ish.example.com/ISHWS/` it is the local PowerShell process (`pwsh.exe - ProcessId:3`) that will trigger your trusted browser to appear for an authentication dance. Unless you ask the LLM to authenticate with client/secret but then you pass a secret to your LLM! In turn if you use the `Stop` or `Restart` decorator in the `mcp.json`, you actually have stopped your PowerShell (`pwsh.exe - ProcessId:3`) process which means you lost your implicit `$ishSession`.
...
* Occasionally the LLM keeps asking to create a session because it identifies it's session as `$null`. The best way out of this loop is to `Restart` and start a new chat.
