...
|--------------------------|-------------------------------------------------|----------------|------------------------|
| [ISHRemote](https://www.powershellgallery.com/packages/ISHRemote/) latest         | PowerShell-5.1/NET-4.8 and PowerShell-7+/NET-6/NET-10+ | master         | [![](https://github.com/RWS/ISHRemote/actions/workflows/continuous-integration.yml/badge.svg?branch=master)](https://github.com/RWS/ISHRemote/actions/workflows/continuous-integration.yml) |
...
## Install on PowerShell 7.6+ (CoreCLR) powered by .NET 10.0+
...
> Detailed authoring rules and reviewer checklists are maintained in [`.github/instructions/`](.github/instructions/) and are auto-injected when editing the relevant paths (`Cmdlets/**/*.cs`, `**/*.Tests.ps1`, `Scripts/**/*.ps1`). The summaries below remain for quick human reference.
...
1. Use Visual Studio to build a `Debug` build. Run an `Invoke-Pester` (minimally Pester v6.0.0) on `C:\GITHUB\ishremote\Source\ISHRemote\Trisoft.ISHRemote`. All steps should be green.
